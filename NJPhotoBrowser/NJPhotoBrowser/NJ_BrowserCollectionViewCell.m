//
//  NJ_BrowserCollectionViewCell.m
//  NJPhotoBrowser
//
...
@interface NJ_BrowserCollectionViewCell()<UIScrollViewDelegate,DetectTapImageVeiwDelegate>
@property (nonatomic, strong) UIScrollView *zoomingScrollView;
@property (nonatomic, strong) NJ_DetectTapImageView *photoImageView;


@end
@implementation NJ_BrowserCollectionViewCell
...
- (NJ_DetectTapImageView *)photoImageView
{
    if (nil == _photoImageView) {
        _photoImageView = [[NJ_DetectTapImageView alloc] initWithFrame:CGRectZero];
...
- (void)displayImage {
    ...
    photoImageViewFrame.size = self.photo.size;
...
- (CGFloat)initialZoomScaleWithMinScale {
    ...
    //即图片宽高比例与scrollview的宽高比非常相近时,适当放大图片使其尽量填满
...
//        zoomScale = MIN(MAX(self.zoomingScrollView.minimumZoomScale, zoomScale), self.zoomingScrollView.maximumZoomScale);
        zoomScale = MIN(zoomScale, self.zoomingScrollView.maximumZoomScale);
...
- (void)handleSingleTap:(CGPoint)touchPoint {    
    [self.photoBrowser toggleNavBar];
}
